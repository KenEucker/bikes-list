...
<template>
    ...
    <CityLayout :city="city" :city-base-url="cityBaseUrl" :breadcrumb="['For Sale', sale.title]">
            ...
            <div v-if="sale.uploads?.length" class="mt-4 flex gap-2 overflow-x-auto">
            ...
            <div v-else-if="sale.attachments?.length" class="mt-4 flex gap-2 overflow-x-auto">
                <img v-for="att in sale.attachments" :key="att.id" :src="att.url" :alt="att.original_name" class="h-48 w-auto rounded object-cover" />
            </div>
            ...
            <section v-if="sale.type === 'full_bicycle'" class="mt-8 rounded-token-md border border-border bg-card p-4">
            ...
            <div class="mt-8 flex flex-wrap gap-3">
                ...
                <template v-if="$page.props.auth.user && (sale.user_id === $page.props.auth.user.id || sale.community_page_id)">
                    <Link v-if="sale.state === 'published'" :href="`${cityBaseUrl}/for-sale/${sale.id}/sold`" method="post" as="button" class="rounded-md bg-amber-600 px-4 py-2 text-sm font-medium text-white hover:bg-amber-700">Mark sold</Link>
                </template>
            </div>
        </div>
    </CityLayout>
