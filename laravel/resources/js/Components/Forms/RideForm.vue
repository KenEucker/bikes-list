...
const initialUploadIds = (r.uploads && Array.isArray(r.uploads) && r.uploads.length)
    ? [r.uploads[0].id]
    : [];
...
<template>
    <form @submit.prevent>
        ...
        <div class="ride-form-accordion-wrapper w-full max-w-4xl">
        <gv-accordion class="govuk-!-width-full">
            <gv-accordion-section heading="Description" id="accordion-description" v-model:expanded="accordionDescriptionExpanded">
                ...
                <div v-if="rideTagsList.length" class="govuk-form-group">
                    <label class="govuk-label">Tags (optional)</label>
                    <div class="govuk-checkboxes govuk-checkboxes--small">
                        <div
                            v-for="[slug, label] in rideTagsList"
                            :key="slug"
            ...
            <gv-accordion-section heading="Contact" id="accordion-contact" v-model:expanded="accordionContactExpanded">
                ...
                <template v-if="isGuest">
                    ...
                    <p class="govuk-body govuk-!-margin-top-2">
                        Your email is required so we can contact you about this ride. You can choose to hide it from the public ride page below.
                    </p>
                </template>
                <gv-checkbox
            ...
            <gv-accordion-section v-if="!isEdit && guidelines.length" heading="Agreements" id="accordion-agreements" v-model:expanded="accordionAgreementsExpanded">
                <div class="rounded-token-md border border-border bg-card p-4">
                    <h2 class="font-medium text-fg">Ride community guidelines</h2>
        ...
        <div class="govuk-button-group govuk-!-margin-top-6">
            <gv-button type="button" variant="primary" :disabled="form.processing" @click="submit">
                {{ isEdit ? 'Save' : 'Submit ride' }}
            </gv-button>
            <a :href="isEdit ? `${cityBaseUrl}/rides/${ride.id}` : `${cityBaseUrl}/rides`" class="govuk-link">Cancel</a>
        </div>
    </form>
</template>

<style scoped>
.ride-form-accordion-wrapper :deep(.govuk-accordion),
.ride-form-accordion-wrapper :deep(.govuk-accordion__section),
.ride-form-accordion-wrapper :deep(.govuk-accordion__section-header),
.ride-form-accordion-wrapper :deep(.govuk-accordion__section-content) {
    width: 100%;
    min-width: 100%;
