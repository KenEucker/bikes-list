...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    computed(() => isEdit.value ? "Edit listing" : "New listing");
    const submitUrl = computed(
      () => isEdit.value ? `${props.cityBaseUrl}/listings/${props.listing.id}` : `${props.cityBaseUrl}/listings`
    );
    const oldInput = props.old || {};
    const listing = props.listing || {};
    const initialUploadIds = listing.uploads && Array.isArray(listing.uploads) ? listing.uploads.map((u) => u.id) : [];
    const form = useForm({
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_gv_select, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<!--[-->`);
            ssrRenderList(__props.listingTypes, (config, key) => {
            ...
            return [
              (openBlock(true), createBlock(Fragment, null, renderList(__props.listingTypes, (config, key) => {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(ssrRenderComponent(_component_gv_select_option, { value: "" }, {
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(`My personal listing`);
                  } else {
                    return [
                      createTextVNode("My personal listing")
                    ];
                  }
                }),
                _: 1
      ...
      _push(`<a${ssrRenderAttr("href", isEdit.value ? `${__props.cityBaseUrl}/listings/${unref(listing).id}` : `${__props.cityBaseUrl}/listings`)} class="govuk-link">Cancel</a></div></form>`);
    };
  }
};
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Components/Forms/ListingForm.vue");
