  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(unref(Head), {
        title: `BikesList – ${__props.city.name} – Moderation – Listings`
      }, null, _parent));
      _push(ssrRenderComponent(_sfc_main$1, {
        city: __props.city,
        "city-base-url": __props.cityBaseUrl,
        breadcrumb: ["Moderation", "Listings"]
      }, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8"${_scopeId}><h1 class="govuk-heading-l"${_scopeId}>Pending listings</h1><p class="mt-2 text-sm text-muted"${_scopeId}>Approve/publish or remove with a note. Actions will be implemented via backend.</p>`);
            _push2(ssrRenderComponent(_sfc_main$2, {
              items: __props.listings.data || [],
              "entity-label": "listings",
              "title-key": "title",
              "show-url-fn": (item) => `${__props.cityBaseUrl}/listings/${item.id}`,
              "approve-url-fn": (item) => `${__props.cityBaseUrl}/moderation/listings/${item.id}/approve`,
              "remove-url-fn": (item) => `${__props.cityBaseUrl}/moderation/listings/${item.id}/remove`,
            ...
            return [
              createVNode("main", { class: "mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8" }, [
                createVNode("h1", { class: "govuk-heading-l" }, "Pending listings"),
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Moderation/Listings.vue");
